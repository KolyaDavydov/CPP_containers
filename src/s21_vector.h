...
namespace s21 {
    ...
    public:
        ...
        explicit Vector(size_type n) : vSize(n), vCapacity(n), vArr(n ? new value_type[n]() : nullptr) {}

        Vector(std::initializer_list<value_type> const &items) : vSize(items.size()), vCapacity(items.size()) {
        ...
        Vector(const Vector &v) : vSize(v.vSize), vCapacity(v.vCapacity), vArr(new value_type[v.vSize]()) {
            CopyEntryVector(v);
        }

        Vector(Vector &&v) noexcept: vSize(v.vSize), vCapacity(v.vCapacity), vArr(v.vArr) {
            v.vSize = 0;
            v.vCapacity = 0;
            v.vArr = nullptr;
        }
...
//methods
        ...
        */
        ...
        If after the operation the new lSize() is greater than old capacity() a reallocation takes place, in which case
        all iterators (including the end() iterator) and all references to the elements are invalidated. If no reallocation
        ...
        */
        ...
        Leaves the capacity() of the vector unchanged (note: the standard's restriction on the changes to capacity is in
         the specification of vector::reserve, see
         */
        void clear() noexcept {
            CleanVectorArr();
            vArr = nullptr;
        ...
        /*
        Exchanges the contents and capacity of the container with those of others. Does not invoke any move, copy, or swap operations on individual elements.
